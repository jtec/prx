...
def main():
    ...
    for week in range(2276, 2000, -1):
        ...
        response = download_url(url, username, password, local_filename=str(scraped_file),
             skip_existing=True, user_agent=None)
        ...
        for row in table_rows:
            dataframe_rows.append({
                ...
                "file_size_bytes": convert_size_to_bytes(row.find("span", {"class": "fileInfo"}).text.split(' ')[-1])
            })
        ...
        print(f"Largest 1-day sp3 file in week {week}: {max_this_week['file_size_bytes']} bytes")
