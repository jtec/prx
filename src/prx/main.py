...
def build_records_polars(
    ...
    def compute_ephemeris_discontinuities_optimized(sat_states_df):
        ...
        sat_states_pl = sat_states_pl.with_columns(
            [
                pl.col("ephemeris_hash")
                .shift(1)
                .over(["sv", "signal"])
                .alias("prev_ephemeris_hash")
            ]
        )
        ...
        sat_states_pl = sat_states_pl.with_columns(
            [
                (
                    ...
                    & pl.col("prev_ephemeris_hash").is_not_null()
                ).alias("after_discontinuity")
            ]
        )

        # Get discontinuity records
        df_new_ephemeris = sat_states_pl.filter(pl.col("after_discontinuity")).drop(
            "after_discontinuity"
        )
        ...
        query_disc = df_new_ephemeris.select(
            ["sv", "signal", "query_time_isagpst"]
        ).unique()
        query_disc = query_disc.with_columns(
            [
                (pl.col("query_time_isagpst") - pl.duration(seconds=10))
                .dt.cast_time_unit("ns")
                .alias("query_time_isagpst")
            ]
        )
    ...
    flat_obs = flat_obs.with_columns(
        [
            pl.when(pl.col("frequency_slot").is_not_null())
            .then(
                pl.col("satellite").str.slice(0, 1)
                + "_L"
                + pl.col("observation_type").str.slice(1, 1)
                + "_"
...
def compute_output_dataframe(
    ...
    existing_expected_cols = [
        col for col in expected_column_order if col in records.columns
    ]
    remaining_cols = [
        col for col in records.columns if col not in expected_column_order
    ]
...
def write_prx_file(
    ...
    return output_writers[output_format](
        prx_header, pl.from_pandas(prx_records), file_name_without_extension
    )
