...
def spp_vt_lsq(df, p_ecef_m, dx_convergence_l2=1e-6, max_iterations=10):
    df = df[df.D_obs_mps.notna()].reset_index(drop=True)
    # Remove satellite velocity projected onto line of sight
    rx_sat_vectors = df[["dx_mps", "dy_mps", "dz_mps"]].to_numpy() - p_ecef_m.T
    ...
    # Initial state estimate we will linearize around
