...
def get_gpst_utc_leap_seconds_from_rinex_header(rinex_file: Path):
    ...
    assert len(ls_before) > 0 and len(ls_before) < 3, f"Unexpected leap seconds {ls_before} in {rinex_file}"
    ls_after = header["LEAP SECONDS"][6:12].strip()
    if ls_after == "":
        return int(ls_before)
    assert len(ls_after) > 0 and len(ls_after) < 3, f"Unexpected leap seconds {ls_after} in {rinex_file}"
    assert ls_after == ls_before, f"Leap second change annoucement in {rinex_file}, this case is not tested, aborting."
    return int(ls_before)
